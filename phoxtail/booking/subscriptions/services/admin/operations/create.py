...
class SubscriptionServiceAdminCreate:
    """
    Admin domain operation for subscription creation.
    """
    ...
    def validate(self, user: User, subscription_type: "SubscriptionType") -> None:
        ...
        if Subscription.objects.filter(
            ...
            raise ValidationError(
                _(
                    f"This user already has an active '{subscription_type.name}' subscription. "
                    "Users cannot have duplicate active subscription types."
                )
            )
