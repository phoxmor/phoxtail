...
that module is still executing. (``phoxtail.users.mcp`` needs no such
line only because ``phoxtail.mcp`` sorts ahead of ``phoxtail.users`` in
its import block, and behind ``phoxtail.agent``.)
"""
