...
class TestUserHasAnyPermission:
    ...
    def test_denies_when_user_lacks_all_permissions(self, policy, user):
        assert (
            policy.user_has_any_permission(
                user, ["access_test_management", "manage_test_items"]
            )
            is False
        )
    ...
    def test_superuser_bypasses_all_checks(self, policy, superuser):
        assert (
            policy.user_has_any_permission(
                superuser, ["access_test_management", "manage_test_items"]
            )
            is True
        )
...
class TestFullPerm:
    ...
    def test_constructs_correct_permission_string(self, policy):
        assert (
            policy._full_perm("access_test_management")
            == "phoxtail_core_testapp.access_test_management"
        )
