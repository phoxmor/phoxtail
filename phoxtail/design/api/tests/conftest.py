"""Fixtures for design v1 API tests.

Run against the real shared ``phoxtail.api`` instance, so the tests exercise
the production wiring: the design router found at ``/design/v1/`` because the
app ships ``phoxtail/design/api/`` declaring ``versions``, with each endpoint
...
def superuser(db, django_user_model):
    return django_user_model.objects.create_superuser(
        ...
        email="design-admin@example.invalid",
        password="irrelevant",
    )
...
def regular_user(db, django_user_model):
    return django_user_model.objects.create_user(
        ...
        email="design-member@example.invalid",
        password="irrelevant",
    )
...
@pytest.fixture
def grant():
    """Give a user one design permission and hand back a fresh instance.
    ...
    def _grant(user, codename: str):
        user.user_permissions.add(Permission.objects.get(content_type__app_label="phoxtail_design", codename=codename))
