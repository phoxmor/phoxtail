"""v1 of the pages API.

Aggregates the generic sub-routers into a single ``Router`` mounted at
``/api/pages/v1/`` by ``phoxtail.api``.

App-specific endpoints (e.g. blog's ``/authors/``) do **not** live under
this prefix — each app owns its own ``/api/<short_label>/v1/`` namespace
and contributes into the pages domain only through the
``page_schema_contributors`` registry in :mod:`contrib`.
"""

from __future__ import annotations

from ninja import Router

from phoxtail.api.pages.v1.body import router as body_router
from phoxtail.api.pages.v1.media import router as media_router
from phoxtail.api.pages.v1.page_types import router as page_types_router
from phoxtail.api.pages.v1.pages import router as pages_router

router = Router()
# /pages/ and /pages/{id}/* live on pages_router.
router.add_router("/pages", pages_router)
# Body endpoints address a page by id: /pages/{id}/body/... — share the prefix.
router.add_router("/pages", body_router)
router.add_router("/media", media_router)
router.add_router("/page-types", page_types_router)
